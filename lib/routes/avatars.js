...
function handle_default(img_status, userId, size, def, req, err, callback) {
  ...
  if (
    ...
    skins.resize_img(
      path.join(__dirname, "..", "public", "images", def + ".png"),
      size,
      function (resize_err, image) {
        callback({
          status: img_status,
          body: image,
          type: "image/png",
...
export default function (req, callback) {
  ...
  const size =
    parseInt(req.url.searchParams.get("size")) || config.avatars.default_size;
  ...
  try {
    helpers.get_avatar(
      ...
      overlay,
      size,
      function (err, status, image, hash) {
        if (err) {
          if (err.code === "ENOENT") {
            // no such file

...
function handle_default(
  ...
  if (
    ...
    fs.readFile(
      path.join(__dirname, "..", "public", "images", def + "_skin.png"),
      function (fs_err, buf) {
...
export default function (req, callback) {
  const raw_type = req.url.path_list[1] || "";
  const rid = req.id;
  const body = raw_type === "body";
  const userId = (req.url.path_list[2] || "").split(".")[0];
  const def = req.url.searchParams.get("default");
  const scale =
    parseInt(req.url.searchParams.get("scale")) || config.renders.default_scale;
  const overlay =
    req.url.searchParams.has("overlay") || req.url.searchParams.has("helm");
  ...
  try {
    helpers.get_render(
      ...
      overlay,
      body,
      function (err, status, hash, image) {
        if (err) {
          if (err.code === "ENOENT") {
            // no such file

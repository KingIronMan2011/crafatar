...
const evMon = monitorEventLoopDelay({ resolution: 10 });
evMon.enable();
const MAX_LAG = 200;
...
function requestHandler(req, res) {
  ...
  // Use POSIX path resolution for cross-platform compatibility
  // This normalizes paths using Unix conventions (no Windows drive letters)
  // and properly handles ".." to prevent path traversal attacks
  req.url.pathname = path.posix.resolve("/", req.url.pathname);
  ...
  if (
    (evMon.mean > MAX_LAG || evMon.percentile(0.95) > MAX_LAG) &&
    !process.env.TRAVIS &&

...
const silent_errors = ["ETIMEDOUT", "ESOCKETTIMEDOUT", "ECONNRESET", "EHOSTUNREACH", "ECONNREFUSED", "HTTPERROR", "RATELIMIT"];
...
export default function(request, response, result) {
  ...
  const etag = "\"" + crc32(result.body || "") + "\"";

  // handle etag caching
  const incoming_etag = request.headers["if-none-match"];

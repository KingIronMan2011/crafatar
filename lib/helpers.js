  ...
  networking.get_skin_info(rid, userId, profile, function(err, url, slim) {
    ...
    if (!err && url) {
      ...
      if (cache_details && cache_details.skin === skin_hash) {
        ...
        const facepath = path.join(config.directories.faces, skin_hash + ".png");
        const helmpath = path.join(config.directories.helms, skin_hash + ".png");
        const skinpath = path.join(config.directories.skins, skin_hash + ".png");

...
const valid_user_id = /^[0-9a-fA-F]{32}$/; // uuid
...
function resume(userId, type, err, hash, slim) {
  const userId_safe = "!" + userId;
  const callbacks = requests[type][userId_safe];
  if (callbacks) {
    ...
    networking.get_profile(rid, userId, function (err, profile) {
      if (err || !profile) {
        ...
        store_skin(
          ...
          profile,
          cache_details,
          function (store_err, skin_hash, slim) {
...
exp.get_image_hash = function (rid, userId, type, callback) {
  cache.get_details(userId, function (err, cache_details) {
    ...
    if (err) {
      ...
      if (
        cache_details &&
        cache_details[type] !== undefined &&
        cache_details.time + config.caching.local * 1000 >= Date.now()
      ) {
        // use cached image
              ...
              cache.update_timestamp(
                ...
                function (err2) {
                  callback(
                    err2 || store_err,
                    4,
                    cache_details && cached_hash,
                    slim,
                  );
                },
              );
            } else {
              const status = cache_details && cached_hash === new_hash ? 3 : 2;
...
exp.get_avatar = function (rid, userId, overlay, size, callback) {
  exp.get_image_hash(
    ...
    userId,
    "skin",
    function (err, status, skin_hash, slim) {
      if (skin_hash) {
      ...
      if (skin_hash) {
        ...
        fs.access(skinpath, function (fs_err) {
          if (!fs_err) {
            ...
            networking.save_texture(
              rid,
              skin_hash,
              skinpath,
              function (net_err, response, img) {
...
exp.get_render = function (rid, userId, scale, overlay, body, callback) {
  exp.get_skin(rid, userId, function (err, skin_hash, status, img, slim) {
    ...
    var renderpath = path.join(
      config.directories.renders,
      [skin_hash, scale, get_type(overlay, body), slim ? "s" : "t"].join("-") +
        ".png",
    );
    fs.access(renderpath, function (fs_err) {
      if (!fs_err) {
        renders.open_render(
          rid,
          renderpath,
          function (render_err, rendered_img) {
        ...
        renders.draw_model(
          ...
          overlay,
          body,
          slim || userId.toLowerCase() === "mhf_alex",
...
exp.get_cape = function (rid, userId, callback) {
  exp.get_image_hash(
    ...
    userId,
    "cape",
    function (err, status, cape_hash, slim) {
      if (!cape_hash) {
        callback(err, null, status, null);
        return;
      }
      const capepath = path.join(config.directories.capes, cape_hash + ".png");
      fs.access(capepath, function (fs_err) {
        if (!fs_err) {
          logging.debug(rid, "cape already exists, not downloading");

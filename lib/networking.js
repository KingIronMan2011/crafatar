...
exp.resetCounter = function() {
  ...
  if (count) {
    const logfunc = count >= config.server.sessions_rate_limit ? logging.warn : logging.debug;
...
exp.get_from_options = function(rid, url, options, callback) {
  const is_session_req = config.server.sessions_rate_limit && url.startsWith(session_url);
  ...
  if (is_session_req && req_count() >= config.server.sessions_rate_limit) {
    ...
    axios.get(url, {
      ...
      const logfunc = code && (code < 400 || code === 404) ? logging.debug : logging.warn;
      ...
      const logfunc = code && (code < 400 || code === 404) ? logging.debug : logging.warn;
      logfunc(rid, url, axiosError.code || code, http.STATUS_CODES[code]);

      const e = new Error(axiosError.message);
      e.name = axiosError.name;
      e.code = axiosError.code || "HTTPERROR";
...
exp.get_uuid_info = function(profile, type, callback) {
  ...
  properties.forEach(function(prop) {
    if (prop.name === "textures") {
      const json = new Buffer.from(prop.value, "base64").toString();
